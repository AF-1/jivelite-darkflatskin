			...
			if server then
				server:userRequest(function(chunk,err)
						if err then
							...
							log:info("Skipping track with ID "..thisTrackID.." without increasing its skip count")
						end
					end,
					self.player:getId(),
					cmd
...
function _setVUmeter(self, vumeter)
	...
	local pathSep = package.config:sub(1,1) -- path separator: Windows = \\, else /
	log:debug('path separator = '..tostring(pathSep))

	local darkFlatSkinPath = string.match(System:findFile('applets'..pathSep..'DarkFlatSkin'..pathSep..'DarkFlatSkinApplet.lua'), '(.*)DarkFlatSkinApplet.lua')
	local vuimagefolder = darkFlatSkinPath..'images'..pathSep..'UNOFFICIAL'..pathSep..'VUMeter'..pathSep
	...
	if pathSep == "\\" then
		os.execute('del "'..vuimagefolder..'vu_analog_25seq_w.png"') -- for Windows: mklink cannot overwrite existing
		os.execute('mklink "'..vuimagefolder..'vu_analog_25seq_w.png" "'..vuimagefolder..'vu_analog_25seq-'..tostring(vumeter)..'.png"')
	else
...
function npVUMeterSelectionShow(self)
	...
	if pathSep == "\\" then help = Textarea("help_text", self:string('NOW_PLAYING_SELECT_VUMETER_HELP_WIN')) end
