...
function getServerData(self, server, thisTrackID)
	...
	local cmd, loopLossless, loopRating, loopComment, loopLyrics, loopBitrate, loopContentType, loopSampleRate, loopSampleSize, loopYear, loopURL, loopReplayGain

	if thisTrackID then
		cmd = {'songinfo', 0 , 100, 'track_id:'..thisTrackID, 'tags:IkoQrRTuyYw'}
	else
		cmd = {'status', '-' , 1, 'tags:IkoQrRTuyYw'}
	end
	...
	server:userRequest(function(chunk,err)
			if err then
				...
				self.setStyles(self, loopLossless, loopRating, loopComment, loopLyrics, loopBitrate, loopContentType, loopSampleRate, loopSampleSize, loopURL, loopYear, loopReplayGain)
			end
		end,
		self.player:getId(),
		cmd
...
function setStyles(self, loopLossless, loopRating, loopComment, loopLyrics, loopBitrate, loopContentType, loopSampleRate, loopSampleSize, loopURL, loopYear, loopReplayGain)
	...
	local bitrate,contentType,sampleRate,sampleSize,streamingService,trackYear,replayGain
	...
	if (settings["displayAudioMetaData"] and contentType) then
		...
		if self.selectedStyle == 'nowplaying_text_only' and replayGain then
...
function npVUMeterSelectionShow(self)
	...
	local npAnalogVUmeters = {'Blue_Glow', 'Jstraw_Dark', 'Jstraw_Dark_Peak', 'Jstraw_Vintage', 'Kolossos_Oval', 'Colored_Circles', 'Orange', 'Tube'}
...
function _createUI(self)
	...
	if (self.windowStyle == "nowplaying_spectrum_text")
	or (string.match(self.windowStyle,"nowplaying_vuanalog_text")) then
