...
applets.DarkFlatSkin.DarkFlatSkinApplet

=head1 DESCRIPTION

800x480 resolution skin.

The DarkFlatSkin applet by AF is based on the JogglerSkin applet, i.e. the work of 3guk, Tarkan Akdam, Justblair & Andy Davison.
...
function skin(self, s, reload, useDefaultSize, w, h, useRatingButtonsGridSkin, hideRepeatButtonGridSkin, hideShuffleButtonGridSkin, useSkipWithoutCountButtonGridSkin)
	local settings = self:getSettings()
...
--------- ICONBAR ---------
	...
	if (settings["hideRepeatButton"] or hideRepeatButtonGridSkin) and (settings["hideShuffleButton"] or hideShuffleButtonGridSkin) then
		if settings["useSkipWithoutCountButton"] or useSkipWithoutCountButtonGridSkin then
	...
	elseif (settings["hideRepeatButton"] or hideRepeatButtonGridSkin) then
		if settings["useSkipWithoutCountButton"] or useSkipWithoutCountButtonGridSkin then
	...
	elseif (settings["hideShuffleButton"] or hideShuffleButtonGridSkin) then
		if settings["useSkipWithoutCountButton"] or useSkipWithoutCountButtonGridSkin then
	...
	if (settings["useRatingButtons"] or useRatingButtonsGridSkin) then
	...
	s.nowplaying = _uses(s.window, {
		...
		npwirelesslinkqualitygroup = {
			order = { 'npwirelesslinkquality' },
			position   = _tracklayout.position,
			border     = _tracklayout.border,
			x = 150 - 40,
			y          = TITLE_HEIGHT,
			h          = 15,
			npwirelesslinkquality =  {
				w          = 80,
				h          = WH_FILL,
				align      = 'center',
				lineHeight = 15,
		...
		npratinggroup = {
			...
			ratingLevel0 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_0_w.png")},
			ratingLevel10 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_10_w.png")},
			ratingLevel20 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_20_w.png")},
			ratingLevel30 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_30_w.png")},
			ratingLevel40 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_40_w.png")},
			ratingLevel50 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_50_w.png")},
			ratingLevel60 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_60_w.png")},
			ratingLevel70 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_70_w.png")},
			ratingLevel80 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_80_w.png")},
			ratingLevel90 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_90_w.png")},
			ratingLevel100 = {	w = 252,
								img = _loadImage(self, "NowPlaying/stars_100_w.png")},
		},
		npratingactiongroupunrate1 = { -- hide in small artwork NP view
			position   = _tracklayout.position,
			border     = {0,0,0,0},
			x          = _tracklayout.x - 40,
		...
		npstatusicongroup = {
			...
			hascsst_0	= {	w = 28,
							img = _loadImage(self, "NowPlaying/csst_0.png")},
			hascsst_1	= {	w = 28,
							img = _loadImage(self, "NowPlaying/csst_1.png")},
			...
			haslyrics_0	= {	w = 28,
							img = _loadImage(self, "NowPlaying/lyrics_0.png")},
			haslyrics_1	= {	w = 28,
							img = _loadImage(self, "NowPlaying/lyrics_1.png")},
	...
	s.nowplaying.npaudiometagroup.pressed = _uses(s.nowplaying.npaudiometagroup)
	s.nowplaying.npgenregroup.pressed = _uses(s.nowplaying.npgenregroup)
	...
	s.nowplaying.npcontrols.pressed = { -- replace bgImg = keyMiddlePressed with nil
		...
		repeatMode      = _uses(s.nowplaying.npcontrols.repeatMode, { bgImg = nil }),
		shuffleAlbum    = _uses(s.nowplaying.npcontrols.shuffleAlbum, { bgImg = nil }),
		shuffleSong     = _uses(s.nowplaying.npcontrols.shuffleSong, { bgImg = nil }),
		shuffleMode      = _uses(s.nowplaying.npcontrols.shuffleMode, { bgImg = nil }),
		shuffleOff      = _uses(s.nowplaying.npcontrols.shuffleOff, { bgImg = nil }),
	...
	s.nowplaying_large_art = _uses(s.nowplaying, {
		...
		npratingactiongroupunrate1 = { hidden = 1}, -- hide in large artwork NP view
		npratingactiongroup1 = {
			x = npX,
			w = 32,
		},
	...
	s.nowplaying_large_art.npcontrols.pressed = {
		rew     = _uses(s.nowplaying_large_art.npcontrols.rew, { bgImg = nil }),
		...
		repeatMode      = _uses(s.nowplaying_large_art.npcontrols.repeatMode, { bgImg = nil }),
		shuffleAlbum    = _uses(s.nowplaying_large_art.npcontrols.shuffleAlbum, { bgImg = nil }),
		shuffleSong     = _uses(s.nowplaying_large_art.npcontrols.shuffleSong, { bgImg = nil }),
		shuffleMode      = _uses(s.nowplaying_large_art.npcontrols.shuffleMode, { bgImg = nil }),
		shuffleOff      = _uses(s.nowplaying_large_art.npcontrols.shuffleOff, { bgImg = nil }),
	...
	s.nowplaying_large_art.npaudiometagroup.pressed = _uses(s.nowplaying_large_art.npaudiometagroup)
	s.nowplaying_large_art.npgenregroup.pressed = _uses(s.nowplaying_large_art.npgenregroup)
	...
	s.nowplaying_art_only = _uses(s.nowplaying, {
		...
		npgenregroup = { hidden = 1 },
		npclockgroup = { hidden = 1 },
		npwirelesslinkqualitygroup = { hidden = 1 },
		npartwork = {
			w = screenHeight,
			position = LAYOUT_NONE,
	...
	s.nowplaying_text_only = _uses(s.nowplaying, {
		...
		npratinggroup    = {
						x = screenWidth/2 - 126 + 6,
                        y = TITLE_HEIGHT + 30 + 60 + 55 + 50 - 6 + 2,
                        nprating =  {
                              w = 252,
                        },
		},
		npratingactiongroupunrate1 = {
						x = screenWidth/2 - 126  + 6 - 70,
                        y = TITLE_HEIGHT + 30 + 60 + 55 + 50 - 6 - 19 + 2,
						w = 70,
		},
		npratingactiongroup1 = {
						x = screenWidth/2 - 126 + 6,
		...
		npratingactiongroup2 = {
						x = screenWidth/2 - 126 + 6 + 50,
                        y = TITLE_HEIGHT + 30 + 60 + 55 + 50 - 6 - 19 + 2,
		},
		npratingactiongroup3 = {
						x = screenWidth/2 - 126 + 6 + 100,
                        y = TITLE_HEIGHT + 30 + 60 + 55 + 50 - 6 - 19 + 2,
		},
		npratingactiongroup4 = {
						x = screenWidth/2 - 126 + 6 + 150,
                        y = TITLE_HEIGHT + 30 + 60 + 55 + 50 - 6 - 19 + 2,
		},
		npratingactiongroup5 = {
						x = screenWidth/2 - 126 + 6 + 200,
                        y = TITLE_HEIGHT + 30 + 60 + 55 + 50 - 6 - 19 + 2,
		},
		npratingactiongroupunrate2 = {
						x = screenWidth/2 - 126 + 6 + 250,
                        y = TITLE_HEIGHT + 30 + 60 + 55 + 50 - 6 - 19 + 2,
		},
		npstatusicongroup = {
						x = screenWidth/2 - 52,
                        y = TITLE_HEIGHT + 30 + 60 + 55 + 50 + 45 + 2,
		},
		npartwork = { hidden = 1 },
	...
	s.nowplaying_text_only.npaudiometagroup.pressed = _uses(s.nowplaying_text_only.npaudiometagroup)
	s.nowplaying_text_only.npgenregroup.pressed = _uses(s.nowplaying_text_only.npgenregroup)
	...
	local _lyricsProgressBarPill = _loadImageTile(self, imgpath .. "UNOFFICIAL/viz_progress_slider.png")
	...
	s.nowplaying_text_lyrics = _uses(s.nowplaying, {
		...
		npprogress = {
			...
			npprogressB = {
				...
				position = LAYOUT_SOUTH,
				horizontal = 1,
				bgImg = false,--_lyricsProgressBarBG,
...
function npButtonSelectorShow(self)
	...
	for i, v in ipairs(tbButtonsLargeArtwork) do
		menu:addItem( {
			...
			check = Checkbox("checkbox",
				function(object, isSelected)
					appletManager:callService("setNowPlayingScreenButtonsDFS", v, isSelected)
					jiveMain:reloadSkin()
				end,
			settings[v]),
		} )
...
function setNowPlayingScreenButtonsDFS(self, button, isSelected)
	local settings = self:getSettings()
	settings[button] = isSelected
	self:storeSettings()
end

function getNowPlayingScreenButtonsDFS(self)
